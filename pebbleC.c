...
void send_message(int arrow){
	DictionaryIterator *iter;
	
	app_message_outbox_begin(&iter);
	dict_write_uint8(iter, STATUS_KEY, arrow);
...
void click(ClickRecognizerRef ref, void *context) {
  ButtonId bid = click_recognizer_get_button_id(ref);
  send_message(bid);
}
